...
- **git 상태: 회원관리 구현+반응성 버그 수정까지 커밋(`b677239`) 완료, 원격 미푸시. 아래 13~15번(락아웃 버그 수정, 페이지네이션/주문 상한, select 아이콘 통일)은 그 이후 작업이라 아직 미커밋**(다음 세션 시작 지점 참고)
- **관리자 회원관리(`MemberManage.tsx`) 실제 구현 완료** — 카페24/Shopify 리서치 후 우리 규모에 맞게 간소화해서 반영(등급/마일리지/휴면회원/최근로그인일은 의도적으로 제외, 이유는 아래):
  ...
  15. **다음 단계**: 매출관리(`SalesManage.tsx`)가 유일하게 남은 완전 스텁 상태
