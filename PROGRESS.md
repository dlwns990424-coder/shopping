...
## 앞으로 해야 할 것 (우선순위 순, 2026-09-09 아홉 번째 세션 갱신)
...
**결정 대기 / 설계 필요**
6. men 상품 hover 이미지(착용컷) — 아직 하나도 없음. 다 채운 뒤 women처럼 기본↔hover 스왑할지 결정 필요(보류 중이던 이미지 최적화도 이거 끝나면 진행)
7. 본인 비밀번호 변경/찾기 기능 — "배포 전에 다시 생각해보자"로 보류
8. 위시리스트 하트→X 아이콘 변경 여부 결정 — 사용자가 "우선 냅두자"로 보류
9. 반품 부분취소(상품 단위) — 보류 중, 별도 설계 필요

**후순위**
10. 상품 이미지 최적화(리사이즈/webp) — men hover 이미지 다 채운 뒤 진행
11. Figma 와이어프레임을 최신 코드에 맞게 재정합 — 재개 요청 있을 때까지 대기
12. 반응형 실기기 정밀 검증 — 이 환경은 iframe 트릭으로만 확인 가능(실제 기기 검증은 아직 안 함)
13. `allowJs` 꺼둔 상태 유지(전체 `.ts`/`.tsx`), 실수로 `.js`/`.jsx` 재유입 주의
