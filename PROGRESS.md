...
## 마지막 갱신
- 날짜: 2026-09-10 (다른 컴퓨터로 전환 예정 — 이 갱신 직후 커밋+push 완료)
...
## 다음 세션 시작 지점 (2026-09-10 기준, 최신 — 다른 컴퓨터에서 이어서 작업 예정)
**코드는 이번 세션 작업분까지 전부 커밋+push 완료.** SQL은 013번까지 전부 사용자가 Supabase SQL Editor에서 실행 확인 완료(밀린 것 없음). 위 "이번 세션 요약"(태블릿 이미지 fix / MEN 착용컷 / 카테고리 카드 호버 / RECOMMENDED 타이틀)이 최신 작업분. 아래는 그 이전(2026-09-09) 세션 요약:
...
## 앞으로 해야 할 것 (우선순위 순, 2026-09-10 갱신)

**배포 전 필수**
1. `products`/`site_content`/`images` 버킷 RLS를 "쓰기는 관리자만"으로 강화 — anon key만 있으면 누구나 데이터 조작 가능한 상태(사이트 점검 세션에서 재확인). `profiles`/`orders`에 이미 있는 `is_admin()` 패턴 재사용 예정

**콘텐츠 입력(코드 작업 아님, 관리자 페이지에서 이미지만 올리면 됨)**
2. `women.hero.image_mobile` 비어있음 — 데스크톱 이미지로 폴백 중
3. `men.hero.image_mobile` 비어있음(UIUX 점검에서 재확인) — 데스크톱 이미지로 폴백 중
4. 홈 이벤트배너 4개 중 2개("데님 컬렉션", "셔츠 & 블라우스") 이미지 미입력 — 홈 화면에 회색 빈 박스로 노출(UIUX 점검에서 발견)
5. 관리자 콘텐츠관리의 "메인 캐러셀 추가 가능한 상품" 목록에 테스트 데이터("상품 추가 테스트", WOMEN) 노출 — 실제 상품 삭제하듯 정리 필요(UIUX 점검에서 발견)

**결정 대기 / 설계 필요**
6. 본인 비밀번호 변경/찾기 기능 — "배포 전에 다시 생각해보자"로 보류
7. 위시리스트 하트→X 아이콘 변경 여부 결정 — 사용자가 "우선 냅두자"로 보류
8. 반품 부분취소(상품 단위) — 보류 중, 별도 설계 필요

**후순위**
9. 상품 이미지 최적화(리사이즈/webp) — men hover 이미지 다 채워졌으니 이제 진행 가능
10. Figma 와이어프레임을 최신 코드에 맞게 재정합 — 재개 요청 있을 때까지 대기
11. 반응형 실기기 정밀 검증 — 이 환경은 iframe 트릭으로만 확인 가능(실제 기기 검증은 아직 안 함)
12. `allowJs` 꺼둔 상태 유지(전체 `.ts`/`.tsx`), 실수로 `.js`/`.jsx` 재유입 주의
...
- ~~상품 상세 태블릿 이미지 크기 fix~~ — 완료(2026-09-10)
