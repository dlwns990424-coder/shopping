...
- **이번 세션 — 배포 전 전체 사이트 QA(브라우저 직접 조작) + 관리자 사이드바 고정 + 콘텐츠관리 죽은 기능 제거**
  ...
  6. **다음에 할 일**: ① 태블릿 취소 버튼 줄바꿈 수정 ② `047_drop_featured_columns.sql` Supabase에서 실행 ③ Figma 기획서 이어서 작성(핵심 기능 — 상품탐색부터) ④ 위 QA에서 테스트 못 한 3가지(드래그정렬/반품처리/콘텐츠이미지저장) 다음에 여유 있을 때 확인 ⑤ 실제로 히어로 편집 폼에서 저장 버튼을 눌러 새 링크 설정 키가 정상 저장되는지는 로드 확인만 했고 저장(write)까지는 테스트 안 함 — 다음에 한 번 확인 권장. **아직 커밋 안 함.**
