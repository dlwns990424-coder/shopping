...
**최우선 (2026-09-15 세션에서 새로 생긴 작업 — 다음 세션 시작 지점 참고)**
0-1. `home.campaign.*` 6개 필드 관리자 편집 UI 연동 (`/campaign/home` 프론트는 완성, 관리자만 남음)
0-2. **(2026-09-15, 잘못 요약됐던 항목 정정)** Men·Women **히어로 섹션**을 클릭하면 각자의 캠페인 상세페이지로 이동하게 만들기 — Home 히어로 클릭 시 `/campaign/home`으로 가는 것과 동일한 패턴(풀스크린 히어로 + 그 성별 관련 상품 나열)을 Men/Women에도 각각 적용. **에디토리얼 서브배너는 대상이 아님** — 서브배너는 지금처럼 이미지+타이틀로만 구성하고 클릭 시 필터링된 상품 목록으로 바로 이동하는 현재 동작이 맞으며 추가 작업 불필요. (원본 결정: 2026-09-14 세션, "차라리 HOME의 HERO SECTION에 에디토리얼 성격을 가진 상세페이지를 넣고... MEN, WOMEN의 HERO SECTION을 클릭하면... 상세페이지로 들어가는데 그 상세페이지의 HERO SECTION엔 비슷한 이미지가 풀 스크린으로 들어가고 그 아래엔 관련된 상품만 나열... MEN, WOMEN의 하단 에디토리얼 배너도... 서브배너 클릭하면 상품 필터링 된게 나오게끔" — 이 메시지가 잘못 요약되어 "서브배너 클릭 시 상세페이지"로 기록되어 있었음, 2026-09-15 세션에서 원본 transcript 대조 후 정정)
0-3. 이미지 소싱 완료 후 사이트 전체 이미지 교체 (히어로/젠더배너/이벤트배너/카테고리카드/에디토리얼배너/home.campaign — 사용자가 직접 탐색 중)
...
9. 홈 이벤트배너 4개 중 2개("데님 컬렉션", "셔츠 & 블라우스") 이미지 미입력 — 홈 화면에 회색 빈 박스로 노출(UIUX 점검에서 발견)
10. 히어로 태블릿 이미지(`home`/`men`/`women.hero.image_tablet`) 신규 필드 — 관리자가 아직 사진 안 올림(비어있으면 모바일 이미지로 폴백되니 급하진 않음)
11. **콘텐츠관리 홈 이벤트배너 목록에 "이벤트 배너 테스트"/"테스트"라는 테스트용 잔재 데이터 2건이 남아있음**(2026-09-14 세션에서 발견, 실제 홈 화면에도 노출될 수 있음) — 관리자 UI에 이벤트배너 삭제 기능 자체가 없어서 지우려면 직접 처리 필요, 사용자가 요청하면 진행하기로 보류 중
