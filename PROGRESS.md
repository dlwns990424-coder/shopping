...
## 다음 세션 시작 지점 (2026-09-11 기준, 최신 — 다른 컴퓨터에서 이어서 작업 예정)
**코드는 이번 세션 작업분까지 전부 커밋+push 완료.** SQL은 `015_hero_tablet_image.sql`까지 사용자 실행 확인함. **`014_orders_self_update_guard.sql`(주문 자기수정 방지 트리거)은 실행했다고는 들었으나 트리거 존재 자체를 API로 확인할 방법이 없었음 — 다음 세션에서 Supabase 대시보드 Database→Triggers에서 `orders_restrict_self_update` 있는지 직접 확인 권장.** **최우선 확인 사항: 상품 상세 로그인 유도 후 사이즈/수량 복원이 실제로 안 된다고 보고받음(위 9번 항목) — 하드리프레시 후 재테스트 결과부터 확인할 것.** UX 세부 피드백 백로그는 코드가 아니라 `피드백내용.txt`(프로젝트 루트, 사용자가 직접 관리)에 있으니 그것도 확인. 위 "이번 세션 요약"이 최신 작업분. 아래는 그 이전(2026-09-09) 세션 요약:
...
## 앞으로 해야 할 것 (우선순위 순, 2026-09-11 갱신)

**배포 전 필수**
1. ~~상품 상세 로그인 유도 후 사이즈/수량 복원 재검증~~ — **완료(2026-09-13)**. React 18 StrictMode 이중 effect 실행으로 복원값이 덮어써지던 버그였음, `useRef` 가드로 수정 후 실제 로그인 흐름 재현해서 복원 확인함
2. **[신규] Supabase 로그인 세션이 로그인 직후 금방 풀리는 현상 확인 필요** — 위 1번 검증 중 발견. `getSession()`이 로그인 직후에도 세션 없음으로 응답, `/mypage` 재방문 시 즉시 로그아웃 상태로 보임. 이 자동화 테스트 환경 특유의 문제일 가능성이 있으나(navigator.locks 등 자동화 브라우저 제약 추정) 실제 사용자 브라우저에서도 재현되는지 다음 세션에서 직접 확인 필요 — 재현되면 실사용에 치명적(로그인 유지 안 됨)이라 최우선
3. `014_orders_self_update_guard.sql` 트리거가 실제로 Supabase에 적용됐는지 대시보드에서 직접 확인(위 참고)
4. `products`/`site_content`/`images` 버킷 RLS를 "쓰기는 관리자만"으로 강화 — anon key만 있으면 누구나 데이터 조작 가능한 상태(사이트 점검 세션에서 재확인). `profiles`/`orders`에 이미 있는 `is_admin()` 패턴 재사용 예정(orders는 014로 부분 대응함, products/site_content/images는 아직)

**콘텐츠 입력(코드 작업 아님, 관리자 페이지에서 이미지만 올리면 됨)**
4. `women.hero.image_mobile` 비어있음 — 데스크톱 이미지로 폴백 중
5. `men.hero.image_mobile` 비어있음(UIUX 점검에서 재확인) — 데스크톱 이미지로 폴백 중
6. 홈 이벤트배너 4개 중 2개("데님 컬렉션", "셔츠 & 블라우스") 이미지 미입력 — 홈 화면에 회색 빈 박스로 노출(UIUX 점검에서 발견)
7. 관리자 콘텐츠관리의 "메인 캐러셀 추가 가능한 상품" 목록에 테스트 데이터("상품 추가 테스트", WOMEN) 노출 — 실제 상품 삭제하듯 정리 필요(UIUX 점검에서 발견)
8. 히어로 태블릿 이미지(`home`/`men`/`women.hero.image_tablet`) 신규 필드 — 관리자가 아직 사진 안 올림(비어있으면 모바일 이미지로 폴백되니 급하진 않음)

**결정 대기 / 설계 필요**
9. 본인 비밀번호 변경/찾기 기능 — "배포 전에 다시 생각해보자"로 보류
10. 위시리스트 하트→X 아이콘 변경 여부 결정 — 사용자가 "우선 냅두자"로 보류
11. 반품 부분취소(상품 단위) — 보류 중, 별도 설계 필요

**후순위**
12. 상품 이미지 최적화(리사이즈/webp) — men hover 이미지 다 채워졌으니 이제 진행 가능
13. Figma 와이어프레임(`Wireframe · User` 페이지)을 최신 코드에 맞게 재정합 — 재개 요청 있을 때까지 대기(기획서 페이지와는 별개)
14. 반응형 실기기 정밀 검증 — 이 환경은 iframe 트릭으로만 확인 가능(실제 기기 검증은 아직 안 함)
15. `allowJs` 꺼둔 상태 유지(전체 `.ts`/`.tsx`), 실수로 `.js`/`.jsx` 재유입 주의
16. UX 세부 피드백(섹션 간 여백, 타이틀 크기, 스크롤 상단 버튼, 리뷰 작성, 장바구니 카운트 뱃지 등) — `피드백내용.txt` 참고, 우선순위는 사용자가 다음 세션에서 정함
