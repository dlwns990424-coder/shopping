...
1. **리뷰 기능 — 실제 로그인해서 진짜 리뷰를 작성해보며 전체 흐름 검증.** 이 작업을 하면서 `ReviewSection.tsx`의 `MOCK_PREVIEW_REVIEWS`(임시 목업 리뷰 2개, `so****`/`ji****`)를 제거할 것 — 관리자 "리뷰 관리"로 확인한 결과 이 상품(롱 울 코트)의 **실제 DB 리뷰는 0개**였고, 지금까지 화면에 보이던 리뷰는 전부 이 목업이었음(위 "이번 세션 요약" 9번 참고). 실제 구매완료 계정으로 로그인 → 리뷰 작성 → 화면 반영까지 실제 흐름으로 검증할 것.
