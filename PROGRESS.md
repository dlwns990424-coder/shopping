...
**배포 전 필수**
1. **상품 상세 로그인 유도 후 사이즈/수량 복원 재검증** — 코드는 있으나(`ProductDetail.tsx`) 사용자가 실제 테스트 시 안 됐다고 보고. 하드리프레시 후 재확인 먼저, 그래도 안 되면 로직 재점검
2. `014_orders_self_update_guard.sql` 트리거가 실제로 Supabase에 적용됐는지 대시보드에서 직접 확인(위 참고)
3. `products`/`site_content`/`images` 버킷 RLS를 "쓰기는 관리자만"으로 강화 — anon key만 있으면 누구나 데이터 조작 가능한 상태(사이트 점검 세션에서 재확인). `profiles`/`orders`에 이미 있는 `is_admin()` 패턴 재사용 예정(orders는 014로 부분 대응함, products/site_content/images는 아직)
